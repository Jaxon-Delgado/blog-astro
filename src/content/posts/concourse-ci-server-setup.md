---
title: Concourse CI Server Setup
date: 2025-11-06
description: A guide on how to setup a web, worker, and database node for Concourse CI to run in a home lab environment.
tags:
  - ci/cd
  - proxmox
  - homelab
  - concourse
image: "[[attachments/concourse-ci-server-setup.png]]"
imageAlt: Concourse Logo
imageOG: false
...
pct create 201 local:vztmpl/ubuntu-24.04-standard_24.04-1.amd64.tar.zst \
    ...
    --password <SUPER_SECRET_PASSWORD> \ # REPLACE THIS 
    --rootfs local-lvm:8 \
...
```bash
pct start 201 && pct exec 201 -- apt update && apt upgrade -y
pct start 202 && pct exec 202 -- apt update && apt upgrade -y
